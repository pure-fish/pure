...
function update_changelog --description "Generate changelog from GitHub releases"
    set --local changelog_file "docs/changelog.md"
    set --local repo "pure-fish/pure"
    ...
    set --local releases_json (curl -s "https://api.github.com/repos/$repo/releases")
    
    if test -z "$releases_json"
